...
| `REJECTED` | `<ulid>` | rawBody (truncated), failedChecks, requestId, receivedAt, expiresAt |

Stats and every bib share a partition key on purpose: one Query returns the stats
item and every athlete together, so `athletesTracked` is **derived by counting
`BIB#` items**, never stored as a counter. A counter is a second source of truth
that can drift; a count cannot.
...
| `TransactionConflict` | Two writers touched the item at once. Says **nothing** about revision | **Retry**, jittered backoff ×3, then 5xx |
...
- `handler.ts` — the ingest Lambda: size check → parse → validate → transaction →
  classify → count.
